package com.mc.common.view.pulluploadmore;
        ...
        post(new Runnable() {
            ...
            public void run() {
                ...
                bottomScrollView.setScrollListener(new MyScrollView.ScrollListener() {
                    ...
                    @Override
                    public void onScroll(int scrollY) {
                        bottomScrollVIewIsInTop = scrollY == 0;
                    }
                ...
                if (bottomScrollVIewIsInTop) {
                    ...
                    if (dy < 0 && currPosition == 1) {
                        if (Math.abs(dy) >= scaledTouchSlop) {
                            if (App.isTop) {//如果viewpager里边的scrollview在最顶部，，就让外边的scrollview获取焦点，否则，让最里边的scrollview获取焦点
                                isIntercept = true;
                            }
                        }
                    }
                    ...
                    if (dy < 0 && currPosition == 1) {
                        if (Math.abs(dy) >= scaledTouchSlop) {
                            if (App.isTop) {//如果viewpager里边的scrollview在最顶部，，就让外边的scrollview获取焦点，否则，让最里边的scrollview获取焦点
                                ...
                                //上下滑动
//左右滑动
                                isIntercept = Math.abs(dy) > Math.abs(dx);
