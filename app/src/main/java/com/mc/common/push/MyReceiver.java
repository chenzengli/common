...
//            ExampleUtil.showToast(printBundle(bundle), App.getContext());
                ...
                for (String key : bundle.keySet()) {
                    if (key.equals(JPushInterface.EXTRA_EXTRA)) {
                        ...
                        try {
                            ...
                            String type_id = json.getString("type_id");
                            switch (type) {
                                case "1":
                                    ToastUtils.showShort("系统消息" + type_id);
                                    break;
                                case "2":
//                                    i = new Intent(context, OrderDetailsActivity.class);
                                    i.putExtra("orderId", type_id);
...
//                                    i = new Intent(context, OrderDetailsActivity.class);
                                    ...
                                    break;
                                case "4":
                                    ToastUtils.showShort("用户通讯" + type_id);
                                    break;
                                case "5":
                                    ToastUtils.showShort("其他" + type_id);
                                    break;
                            }
    ...
    private void init(Context context) {
        ...
        bean.alias = SPUtils.getInstance().getString(Constant.UID);
        bean.isAliasAction = true;
        TagAliasOperatorHelper.sequence++;
        TagAliasOperatorHelper.getInstance().handleAction(context, TagAliasOperatorHelper.sequence, bean);
    }
